# Movie Night Project

Movie Night Project is a small web app/PWA project that helps users organize movies they want to watch.

Project Overview

Movie Night Planner is a small Progressive Web App that helps users keep track of movies they want to watch. Users can add movies, organize them by genre, track whether they have watched them, and manage their personal watchlist in a clean interface.

The goal of this project is to create a simple web application that demonstrates core front-end development concepts while laying the groundwork for future improvements such as offline support, persistent storage, and installability as a PWA.

Purpose

This app is designed for people who enjoy movies and want a simple way to manage a personal watchlist. It connects to a real-world interest while staying small enough to develop over several weeks.

Technologies / Tools
HTML5 for page structure
CSS3 for styling and responsive layout
JavaScript for interactivity
Local Storage for saving movie data
Manifest.json for PWA setup
Service Worker for offline support later
GitHub for source control and submission
VS Code for development

## Features
- Add movies to a watchlist
- Select genre
- Mark movies as watched
- Delete movies from the list

## Tools 
- HTML
- CSS
- JavaScript
- GitHub



## Author
Josh Grenninger
